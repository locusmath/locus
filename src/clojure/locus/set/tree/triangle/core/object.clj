(ns locus.set.tree.triangle.core.object
...
(derive SetTriangle :locus.set.logic.structure.protocols/copresheaf)
...
; Successor quivers for set triangles
(defmethod successor-quiver SetTriangle
  [^SetTriangle triangle, i]

  (cond
    (= i '()) (identity-function (triangle-target triangle))
    (= i '(0)) (postfunction triangle)
    (= i '(0 0)) (prefunction triangle)))

(defmethod get-set SetTriangle
  [^SetTriangle triangle, i]

  (case (count i)
    0 (triangle-target triangle)
    1 (triangle-middle triangle)
    2 (triangle-source triangle)))

(defmethod get-function SetTriangle
  [^SetTriangle triangle, [source path]]

  (case [(count source) (count path)]
    [0 0] (identity-function (triangle-target triangle))
    [1 0] (identity-function (triangle-middle triangle))
    [2 0] (identity-function (triangle-source triangle))
    [1 1] (postfunction triangle)
    [2 1] (prefunction triangle)
    [2 2] (compfunction triangle)))
...
(defmethod to-triangle :locus.set.logic.core.set/universal
  [rel] (relation-to-triangle rel))

(defmethod to-triangle :locus.set.logic.structure.protocols/invertible-set-function
  [invertible-function]

  (->SetTriangle
    (underlying-function (inv invertible-function))
    (underlying-function invertible-function)))
