(ns locus.set.quiver.ternary.core.object
...
(derive TernaryQuiver :locus.set.quiver.structure.core.protocols/ternary-quiver)
...
(defmethod nth-component :locus.set.quiver.structure.core.protocols/ternary-quiver
  [quiver edge i]
...
(defmethod get-set :locus.set.quiver.structure.core.protocols/ternary-quiver
  [quiver x]
...
(defmethod get-function :locus.set.quiver.structure.core.protocols/ternary-quiver
  [quiver x]
...
(defmethod to-ternary-quiver :locus.set.logic.core.set/universal
  [coll] (relational-ternary-quiver coll))

(defmethod to-ternary-quiver :locus.set.logic.structure.protocols/set-function
  [func]
...
(defmethod product :locus.set.quiver.structure.core.protocols/ternary-quiver
  [& quivers]

  (apply ternary-quiver-product quivers))

(defmethod coproduct :locus.set.quiver.structure.core.protocols/ternary-quiver
  [& quivers]
...
; Set images for ternary quivers
(defmethod image
  [:locus.set.quiver.structure.core.protocols/ternary-quiver :locus.set.logic.core.set/universal]
  [quiver coll]
...
(defmethod inverse-image
  ...
  (ternary-quiver-set-inverse-image quiver coll))
...
; Domain binary relations of algebraic ternary quivers and their generalisations
(defmulti domain-binary-relation type)

(defmethod domain-binary-relation :locus.set.quiver.structure.core.protocols/ternary-quiver
  [ternary-quiver]

  (set
    (map
      (fn [morphism]
        (list (first-component ternary-quiver morphism) (second-component ternary-quiver morphism)))
      (morphisms ternary-quiver))))

; Ontology of ternary quivers
(defmulti ternary-quiver? type)

(defmethod ternary-quiver? :locus.set.quiver.structure.core.protocols/ternary-quiver
  [quiv] true)
...
(defmethod thin-ternary-quiver? :locus.set.quiver.structure.core.protocols/thin-ternary-quiver
  [quiv] true)
...
(defmethod at-quiver? :locus.set.quiver.structure.core.protocols/at-quiver
  [quiv] true)
...
(defn magma-quiver?
  ...
  (and
    (at-quiver? quiver)
    (equal-universals?
      (domain-binary-relation quiver)
      (complete-relation (objects quiver)))))
