...
; are elements of a class of distributive lattices that generalises boolean algebras. Thusly, we can
; treat multisets like sets that duplicate elements, which can be defined by indexing over multiplicities.
; This file implements the extension of the Clojure collection libraries to include Multisets, by
; providing a basic implementation of a multiset class. At the same time, we provide several generalized
; classes that extend Clojure's built in support for sets.
...
; The overall performance of the seqable universal system
; will be greatly improved by making choice into a function, but
; in doing so this will require a great refactoring of all uses of
; the seqable universal datatype.
...
(defmulti seqable-universal? type)
(defmethod seqable-universal? SeqableUniversal [a] true)
(defmethod seqable-universal? :default [a] (set? a))

(defmulti intrinsic-universal? type)
(defmethod intrinsic-universal? Universal [a] true)
(defmethod intrinsic-universal? :default
  [a]
  (or
    (seqable-universal? a)
    (fn? a)
    (= (type a) clojure.lang.MultiFn)))
...
(defn universal?
  ...
  (or
   ...
   (and
    (multiset? coll)
    (= (count coll) (count (set (keys (.multiplicities coll))))))))

; Seqable intervals
(defn seqable-interval
  [a b]

  (SeqableUniversal.
    (fn [i] (and (integer? i) (<= a i) (< i b)))
    (range a b)
    {:count (- b a)}))
...
(defmethod seqable-universal? Upto
  [x] true)
...
(defmethod seqable-universal? PowerSet
  [a] true)
...
; Power sets are intersection closed
...
; Mapping from multisets to their signatures
(defn signature
  [coll]

  (if (seqable-universal? coll)
    (repeat-multiset (count coll) 1)
    (Multiset. (frequencies (vals (.multiplicities coll))))))

; Multiplicities and the support
(defn multiplicity
  [coll key]

  (cond
    (set? coll) (if (contains? coll key) 1 0)
    (seqable-universal? coll) (if (coll key) 1 0)
    :else (or (get (.multiplicities coll) key) 0)))
...
; Distributive decomposition of the natural numbers
(deftype Ordinal [n]
  clojure.lang.Seqable
...
(defmethod seqable-universal? Ordinal [a] true)
...
; Cardinalities classification of sets
          ...
          (present-graph [edges]
            (str
             ...
             (apply
              ...
              (map
               (fn [i]
                 (str (stringify (first i)) " ; "))
               (filter singular-universal? edges)))
             (apply
              str
              (map
               (fn [edge]
                 (str (stringify (first edge)) " -- " (stringify (second edge))))
               (filter size-two-universal? edges)))
             "} "))
          (present-digraph [edges]
            (str
